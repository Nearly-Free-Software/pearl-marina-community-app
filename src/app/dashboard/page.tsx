...
import { CalendarDays, Home, LogOut, QrCode, ShieldCheck } from "lucide-react";
    ...
    <main className="min-h-screen">
      ...
      <div className="mx-auto w-full max-w-5xl px-5 py-8 sm:px-8 sm:py-12">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Card><CardHeader><div className="mb-2 grid size-10 place-items-center rounded-full bg-secondary"><Home className="size-5" /></div><CardTitle>Your account</CardTitle><CardDescription>Community access and contact information.</CardDescription></CardHeader><CardContent className="space-y-4"><div><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Email</p><p className="mt-1 break-all text-sm">{profile.email}</p></div><div><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Access</p><p className="mt-1 flex items-center gap-2 text-sm"><ShieldCheck className="size-4 text-primary" />Active</p></div></CardContent></Card>
          {canInviteGuests(profile.role) ? (
            <Card><CardHeader><div className="mb-2 grid size-10 place-items-center rounded-full bg-secondary"><QrCode className="size-5" /></div><CardTitle>Guest access</CardTitle><CardDescription>Create a time-limited QR pass for a visitor.</CardDescription></CardHeader><CardContent className="grid gap-3"><Button asChild><Link href="/visitors/new">Invite a guest</Link></Button><Button asChild variant="outline"><Link href="/visitors">Manage guest passes</Link></Button></CardContent></Card>
          ) : (
