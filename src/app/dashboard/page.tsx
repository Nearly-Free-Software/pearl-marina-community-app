...
import { CalendarDays, ClipboardCheck, Home, Link2, LogOut, ShieldCheck } from "lucide-react";
      ...
      <div className="mx-auto w-full max-w-5xl px-5 py-8 sm:px-8 sm:py-12">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          {canInviteGuests(profile.role) ? (
            <Card><CardHeader><div className="mb-2 grid size-10 place-items-center rounded-full bg-secondary"><Link2 className="size-5" /></div><CardTitle>Guest access</CardTitle><CardDescription>Create a time-limited visitor link.</CardDescription></CardHeader><CardContent className="grid gap-3"><Button asChild><Link href="/visitors/new">Invite a guest <LinkStatus label="Opening guest form" /></Link></Button><Button asChild variant="outline"><Link href="/visitors">Manage guest passes <LinkStatus label="Opening guest passes" /></Link></Button></CardContent></Card>
          ) : (
