                  ...
                  {application.id_required ? (
                    <div className="space-y-3 rounded-lg border bg-secondary/30 p-4">
                      ...
                      <dl className="grid gap-3 text-sm sm:grid-cols-2">
                        ...
                        <div><dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Computer-assisted name suggestion</dt><dd className={`mt-1 ${application.id_ocr_suggested_name && application.id_ocr_suggested_name.toLocaleLowerCase() !== application.full_name.toLocaleLowerCase() ? "font-medium text-amber-800" : ""}`}>{application.id_ocr_suggested_name ?? (application.id_ocr_status === "failed" ? "Name suggestion unavailable — inspect closely" : "No name suggested — inspect closely")}</dd></div>
                      </dl>
