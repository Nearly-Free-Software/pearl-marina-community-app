                  ...
                  {application.id_required ? (
                    <div className="space-y-3 rounded-lg border bg-secondary/30 p-4">
                      ...
                      <dl className="grid gap-3 text-sm sm:grid-cols-2">
                        ...
                        <div><dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Computer-assisted name suggestion</dt><dd className={`mt-1 ${application.id_ocr_suggested_name && application.full_name && application.id_ocr_suggested_name.toLocaleLowerCase() !== application.full_name.toLocaleLowerCase() ? "font-medium text-amber-800" : ""}`}>{application.id_ocr_suggested_name ?? (application.id_ocr_status === "failed" ? "Name suggestion unavailable — inspect closely" : "No name suggested — inspect closely")}</dd></div>
                      </dl>
        ...
        {reviewed.length ? (
          <section className="mt-10" aria-labelledby="history-heading">
            ...
            <div className="grid gap-3">{reviewed.map((application) => (
              <Card key={application.id}><CardContent className="flex flex-col gap-4 pt-6 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{application.anonymized_at ? "Deleted account" : application.full_name}</p>{application.anonymized_at ? <p className="mt-1 text-sm text-muted-foreground">Personal details erased</p> : <p className="mt-1 text-sm text-muted-foreground">{application.sub_community} · Unit {application.unit_number}</p>}<p className="mt-1 text-xs text-muted-foreground">{application.status === "approved" ? "Approved" : application.status === "expired" ? "Expired" : "Rejected"} {application.reviewed_at ? formatKampalaDateTime(application.reviewed_at) : ""}</p>{application.id_required ? <p className="mt-1 text-xs text-muted-foreground">ID {application.id_deleted_at ? "deleted" : application.id_delete_after ? `available until ${formatKampalaDateTime(application.id_delete_after)}` : "retention pending"}</p> : null}</div>{application.status === "approved" && !application.invitation_sent_at && !application.anonymized_at ? <form action={approveHomeownerApplication.bind(null, application.id)}><PendingButton variant="outline" pendingLabel="Sending…"><MailWarning className="size-4" />Retry email</PendingButton></form> : <span className="inline-flex items-center gap-2 text-sm"><Home className="size-4 text-primary" />{application.anonymized_at ? "Account deleted" : application.status === "approved" ? "Registered" : "Not registered"}</span>}</CardContent></Card>
            ))}</div>
          </section>
        ) : null}
      </div>
